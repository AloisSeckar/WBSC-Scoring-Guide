...
# Auty a postupy běžců

## Aut

Stejně jako běžící pálkař může být i běžec na metách standardně autován dvěma způsoby - zašlápnutím mety (buďto té před sebou, na kterou je nucen postupovat spoluhráči za sebou, nebo méně často té, na kterou se měl vrátit po chycení odpalu ze vzduchu) nebo tečováním, které může proběhnout v blízkosti mety při (obvykle) pokusu o slajd nebo někde mezi metami při tzv. run-downu. 
Princip zápisu je stejný, jen kolečko označující aut bude v těchto případech menší. Můžete nicméně maximálním způsobem využít zbylý prostor, není třeba cpát znaky pouze k následující metě (zejména při autech na 2. metě je to častá „chyba“ zapisovatelů-začátečníků). Při jednoduchých autech, které si nyní ukazujeme, nemusí být důvod patrný, ale až u složitějších rozeher začnou přibývat další znaky a symboly, tak najednou více prostoru pro zápis sami oceníte.
...
Aut běžce může mít důsledek na posouzení akce pálkaře, ale to si probereme až za chvíli v pokročilých situacích (LINK).
...
Ti, kteří se učili českou variantu zápisu softballu, budou tímto konceptem možná překvapeni, neboť jsou zvyklí při odpalu na devět psát u postupů rovněž devítky - říkají tím „postup nastal v důsledku odpalu na devět“ místo „postup nastal v důsledku akce druhého pálkaře v lineupu“. Proč je lepší druhá varianta se ukáže ve chvíli, kdy se útočícímu týmu podaří slepit třeba tři „odpaly na devět“ za sebou a zápis se najednou hemží samými devítkami. Ještě horší to bylo s metami zdarma - občas si nadhazovač vybere slabší chvilku a hodí třeba čtyři za sebou. Český softballový zápis znázorňuje nucené postupy běžců po BB pomocí bb psanými malými písmeny. Z takového zápisu je rázem chaos.

Naproti tomu předpokládejme 4 BB za sebou na první čtyři pálkaře ve WBSC variantě zápisu. První pálkař bude mít BB pouze u své první mety. Posun na druhou metu „zařídil“ svým BB druhý pálkař, tedy tam napíšeme 2, ke třetí metě 3 a na domácí metu 4. Ačkoliv zápas samotný zřejmě za moc nestojí, alespoň se dá mnohem lépe vyznat v zápisu z něj.

Navíc díky zápisu pálkařského pořadí u doběhů domů ihned vidíme konkrétně, čí zásluha je postup běžce na domácí metu, a který z pálkařů má tudíž dostat do statistik stažený bod (RBI - runs batted in)[^3]. Nemusíme si vypomáhat hvězdičkami či jinými dodatečnými symboly.

## WP/PB

Při nadhozu se občas něco nepovede - buďto nadhazovač míč hodně špatně hodí nebo ho catcher nezvládne chytit. Míč skončí někde na zemi mimo bezprostřední dosah catchera a jeden nebo více běžců využije situaci a postoupí (nejméně) o metu dál. To je poměrně častý scénář, jehož výsledkem je buďto WP (wild pitch) nebo PB (passed ball). WP je vina nadhazovače, PB je specifický druh polařské chyby catchera.
...
Zatím jsme si řekli teorii zápisu, ještě však musíme vydefinovat, kdy psát chybu nadhozu WP a kdy chybu catchera PB. Špatná zpráva - musíte to rozhodnout podle sebe a často bude záležet výlučně na vašem citu. Jediné arbitrární pravidlo, které na to máme, je, že jakmile se míč před metou nebo na úrovni mety dotkne země, tak je to vždycky wild pitch. Ano, catcheři se učí míče zaklekávat a spoustu „jednoduchých“ situací by měli umět vyřešit. Je to super, když se jim to podaří. Ale z pohledu zapisovatele to není náplň jejich práce. Nadhazovač sice má házet složité nadhozy, aby to ztížil pálkařům, ale při tom musí pořád házet „rozumně“, tj. v běžném dosahu svého catchera.

To mějte na paměti při posuzování. Jakmile jde catcher hodně do strany nebo musí vystřelit rukavicí (a někdy i celým tělem) nahoru, tak to passed ball být nemá. Obecně platí, že dlouhodobě jsou minimálně 4/5 situací WP a pouze výrazná menšina míčů je skutečně špatně zahraná catcherem, takže má být PB. Moje dlouholetá zkušenost říká, že u nás v ČR je mentální nastavení zapisovatelů extrémně vychýleno v neprospěch catcherů. Podle obecného mínění komunity mají chytat „všechno“ (protože na to přece trénují) a teprve, když je to opravdu hodně mimo, jsou někteří ochotni uznat, že tohle vážně nešlo. Nemá to tak být. 
Polední slovo u každé konkrétní rozehry budete mít vždycky vy. Užívejte tuto svou moc moudře.
...
[^1]: **POZOR:** červená hvězdička v tomto a mnoha budoucích příkladech <u>**neznamená**</u> žádný speciální zapisovatelský symbol. Je to pouze obrazné vyjádření, že běžec tuto metu již nějak získal - a nás v tuto chvíli nezajímá jak, protože to pro aktuálně popisovanou situaci není důležité.
[^2]: V případě slowpitche, kde pálí více hráčů, je možné i 10
[^3]: O vyhodnocování této statistiky více později (LINK)
